...
Task environment definition:
    ...
    2. Action execution model (collision mechanism):
        - agent-obstacle: reach and disappear.
        - agent-agent (within swarm homogeneous): reach and disappear.
        - pursuer-evader:
            - evader: reach and disappear.
            - pursuer: reach and disappear.
...
class MatrixWorld(MatrixWorldTwoSwarmConcurrentBase):
    ...
    def delete_collided_evaders(self):

        # Delete.

        self.evaders_last_active_index = self.evaders_active_index.copy()

        self.evaders_active_index = self.evaders_active_index[~self.evader_collision_status]

        all_positions_to_delete = self.evaders[self.evader_collision_status]

        all_positions_to_delete_in_padded = all_positions_to_delete + self.fov_offsets_in_padded

        # All agents in this position are all collided and should be deleted.

        self.padded_env_matrix[all_positions_to_delete_in_padded[:, 0],
                               all_positions_to_delete_in_padded[:, 1], 0] = 0

        self.evaders_deleted_list += self.evaders[self.evader_collision_status].tolist()

        self.evaders = self.evaders[~self.evader_collision_status]

        # Update.

        self.n_evaders = len(self.evaders)

    def delete_collided_pursuers(self):

        # Delete.

        self.pursuers_last_active_index = self.pursuers_active_index.copy()

        self.pursuers_active_index = self.pursuers_active_index[~self.pursuer_collision_status]

        all_positions_to_delete = self.pursuers[self.pursuer_collision_status]
        ...
        self.padded_env_matrix[all_positions_to_delete_in_padded[:, 0],
                               all_positions_to_delete_in_padded[:, 1], 1] = 0

        self.pursuers_deleted_list += self.pursuers[self.pursuer_collision_status].tolist()

        self.pursuers = self.pursuers[~self.pursuer_collision_status]

        # Update.

        self.n_pursuers = len(self.pursuers)
    ...
    def step(self, actions_pursuer=None, actions_evader=None):
        ...
        self.delete_collided_evaders()

        self.delete_collided_pursuers()
    ...
    def collision_mechanism(self, actions_evader, actions_pursuer):
        ...
        from_positions_evader, desired_positions_evader = \
            self.collision_mechanism_agent_obstacle_reach(actions_evader, is_evader=True)

        from_positions_pursuer, desired_positions_pursuer = \
            self.collision_mechanism_agent_obstacle_reach(actions_pursuer, is_evader=False)
    ...
    def update_game_status(self):
        ...
        self.evader_collision_status = self.evader_collision_status_with_obstacles + \
                                       self.evader_collision_status_with_evaders + \
                                       self.evader_collision_status_with_pursuers

        self.pursuer_collision_status = self.pursuer_collision_status_with_obstacles + \
                                        self.pursuer_collision_status_with_pursuers + \
                                        self.pursuer_collision_status_with_evaders

        self.capture_rate = \
            (self.n_evaders_initial - self.n_evaders + sum(self.evader_capture_status)) / self.n_evaders_initial

        self.evader_survive_rate = (self.n_evaders - sum(self.evader_collision_status)) / self.n_evaders_initial

        self.pursuer_survive_rate = (self.n_pursuers - sum(self.pursuer_collision_status)) / self.n_pursuers_initial

        self.evaders_done = self.evader_collision_status.copy()

        self.pursuers_done = self.pursuer_collision_status.copy()

        self.game_done = self.evaders_done.all() or self.pursuers_done.all()
